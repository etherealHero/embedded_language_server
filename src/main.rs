...
struct ServerState {
    pool: Pool,
    config: tokio::sync::RwLock<lsp_proxy::Config>,
...
impl Server {
    ...
    fn parse_config(&self, p: &lsp::InitializeParams) -> Result<(PathBuf, lsp_proxy::Config)> {
        ...
        let err = |e| anyhow!("parse config error: {e}\n{CONFIG_HELP}");
        let config: lsp_proxy::Config = toml::from_str(&raw_config).map_err(err)?;
    ...
    async fn startup(&self, p: &lsp::InitializeParams) -> Result<()> {
        ...
        let (config_path, config) = config.inspect_err(|e| error!("parse config error: {e}"))?;
...
fn create_hash(data: &str) -> String {
    use sha2::Digest;
...
fn verify_hash(data: &str, hash: &str) -> bool {
...
async fn main() {
    ...
    match cli.command {
        Some(Commands::Lsp) => {
            debug!("debug level logging enabled"); // if cli.debug
            run_service().await;
        }

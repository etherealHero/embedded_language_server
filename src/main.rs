...
static CONFIG_HELP: &str = concat!(
    "config file should be a struct:\n",
    "\t<host> [String] - server instance name\n",
    "\t<database> [String] - database instance name\n",
    "\t<sign> [String] - injection protection for <get_symbols_query> generated by --sign <FILE> command\n",
    "\t<get_symbols_query> [String] - sql query which returns dataset with struct:\n",
    "\t\tIdentifier VARCHAR UNIQUE NOT NULL,\n",
    "\t\tHoverInfo VARCHAR,\n",
    "\t\tDefinitionInfo VARCHAR,\n",
    "\t\tDefinitionFileExtension VARCHAR\n",
    "\t<case_sensitive> [Boolean?] - optional flag to search Identifier with case sensitive or not (defaults true)",
);
...
impl Server {
    ...
    fn parse_config(&self, p: &lsp::InitializeParams) -> Result<(PathBuf, Config)> {
        ...
        let config: Config = toml::from_str(&raw_config).map_err(err)?;
...
fn sign_config(config_path: &String) {
    ...
    let path = path.map(|p| dunce::canonicalize(dunce::simplified(&p)));

    let Some(Ok(config_path)) = path else {
    ...
    let try_parse: std::result::Result<Config, _> = toml::from_str(&raw_config);
    ...
    let sign_config = toml::to_string(&config).unwrap();

    let Err(err) = std::fs::write(config_path, sign_config) else {
...
fn init_registry(debug_level_in_release: bool) {
    ...
    let registry = tracing_subscriber::registry().with(cfg_select! {
        debug_assertions => LevelFilter::DEBUG,
        _ => if release_debug_level { LevelFilter::DEBUG } else { LevelFilter::INFO }
    });
    ...
    let layer = match debug_level_in_release {
...
async fn main() {
    let mut cmd = clap::command!()
        .arg(
            clap::Arg::new("lsp")
                .long("lsp")
                .help("Start language server service (stdio transport)")
        ...
        .arg(
            clap::Arg::new("sign")
                .long("sign")
                .value_name("FILE")
                .help("Sign config file"),
        )
        .arg(
            clap::Arg::new("stdio") // vscode language Client provide --stdio by default
                .long("stdio")
                .hide(true)
                .action(clap::ArgAction::SetTrue),
        );

    let cli = cmd.clone().get_matches();

    init_registry(cli.get_flag("debug"));

    if let Some(config_path) = cli.get_one::<String>("sign") {
        sign_config(config_path);
    } else if cli.get_flag("lsp") {
        run_service().await;
    } else {
        let app = clap::crate_name!();
        error!("expect --sign or --lsp option (See '{app} --help')",);
